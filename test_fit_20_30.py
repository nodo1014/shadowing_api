...
if result:
    print(f"✓ Success: {output_dir}/shorts_fit_20_30.mp4")
    
    # Extract preview at 5 seconds
    cmd = [
        'ffmpeg', '-i', str(output_dir / "shorts_fit_20_30.mp4"),
        '-ss', '00:00:05', '-vframes', '1', '-q:v', '2',
        '-y', str(output_dir / "preview_fit_20_30.png")
    ]
    subprocess.run(cmd, capture_output=True)
    print(f"✓ Preview saved: {output_dir}/preview_fit_20_30.png")
    
    # Also extract at 0 seconds to see "자막 없이 듣기"
    cmd = [
        'ffmpeg', '-i', str(output_dir / "shorts_fit_20_30.mp4"),
        '-ss', '00:00:00', '-vframes', '1', '-q:v', '2',
        '-y', str(output_dir / "preview_fit_20_30_start.png")
    ]
    subprocess.run(cmd, capture_output=True)
    print(f"✓ Preview saved: {output_dir}/preview_fit_20_30_start.png")
