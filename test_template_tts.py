#!/usr/bin/env python3
"""
Template TTS 테스트 스크립트
무자막 → 블랭크 → 블랭크+한글 → 영한자막 → 무자막 → TTS해설
"""
import logging
...
def test_template_tts():
    ...
    # 출력 경로
    output_path = "output/test_template_tts/template_tts_test.mp4"
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    
    # 인코더 생성
    encoder = TemplateVideoEncoder()
    
    logger.info("Testing template_tts...")
    ...
    if result:
        logger.info("✅ Template TTS 테스트 성공!")
        ...
        logger.error("❌ Template TTS 테스트 실패!")
