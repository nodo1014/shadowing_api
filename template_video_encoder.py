...
class TemplateVideoEncoder(VideoEncoder):
        ...
        try:
            ...
            for clip_config in template['clips']:
                for i in range(clip_config['count']):
                    ...
                    # Get subtitle file for this clip type
                    ...
                    if not self._encode_clip(media_path, temp_clips[-1],
                        ...
                        raise Exception(f"Failed to create {clip_config['type']} clip")
                    ...
                    if save_individual_clips and clip_base_dir:
                        self._save_individual_clip(temp_clips[-1], clip_base_dir,
                                                 clip_config['type'], i + 1, clip_number)
                    
                    print(f"Created {clip_config['type']} clip {i+1}/{clip_config['count']}")
            ...
            print(f"Error creating video from template: {e}")
            return False
    ...
    def _prepare_subtitle_files(self, subtitle_data: Dict, template_name: str) -> Dict[str, str]:
        ...
        if template_name == "type_1":
            # Type 1: full subtitle only
            full_ass = tempfile.NamedTemporaryFile(suffix='_full.ass', delete=False)
            full_ass.close()
            self.subtitle_generator.generate_full_subtitle(subtitle_data, full_ass.name)
            subtitle_files['full'] = full_ass.name
            
        elif template_name == "type_2":
            # Type 2: blank and full subtitles
            # Blank subtitle
            ...
            # Full subtitle with keywords
            full_ass = tempfile.NamedTemporaryFile(suffix='_full.ass', delete=False)
            full_ass.close()
            self.subtitle_generator.generate_full_subtitle(subtitle_data, full_ass.name, with_keywords=True)
            subtitle_files['full'] = full_ass.name
            
        elif template_name == "type_3":
            # Type 3: korean only and full subtitles
    ...
    def _save_individual_clip(self, clip_path: str, base_dir: Path, 
        ...
        # Create subdirectory for clip type
        type_map = {
            "no_subtitle": "1_nosub",
            "blank_subtitle": "2_blank",
            "both_subtitle": "3_both",
            "full_subtitle": "4_full"
        }
        
        sub_dir = base_dir / type_map.get(clip_type, clip_type)
        sub_dir.mkdir(exist_ok=True)
        
        dest_file = sub_dir / f"clip_{clip_number}_{index}.mp4"
        shutil.copy2(clip_path, str(dest_file))
        print(f"Saved: {dest_file.relative_to(base_dir)}")